...
# the files are located in ../raw-data/lifetables/States/[STATE CODE]/[LT NAME].csv
# (we can't distribute these data - you'll have to download them from https://usa.mortality.org/)
usmd.data.dir <- here("../raw-data/")

# this is where we'll save outputs
out.dir <- here("replication-out")
```

```{r}
state_codes <- list.files(file.path(usmd.data.dir, 'lifetables', 'States'))

# quick function to get the life table for a state
get_lt <- function(code, type='b', fmt="1x1") {
  fullpath <- glue::glue("{usmd.data.dir}/lifetables/States/{code}/{code}_{type}ltper_{fmt}.csv")
  this_lt <- read_csv(fullpath, 
